...
class Scraper(object):
    ...
    def get_artist_info(self, artist: Artist) -> dict:
        ...
        # Get similar artists
        response = r.get(
            f"http://ws.audioscrobbler.com/2.0/?method=artist.getsimilar&artist={artist_name}&api_key={self.API_KEY}&format=json"
        )
        if response.status_code != 200:
            raise IOError(f"IOError: {r.status_codes._codes[response.status_code]}")
        resp_json = response.json()["similarartists"]["artist"]
        artist_info["similar_artists"] = [
            similar_artist["name"] for similar_artist in resp_json
        ]

        # Get top tags
        response = r.get(
            f"http://ws.audioscrobbler.com/2.0/?method=artist.getTopTags&artist={artist_name}&api_key={self.API_KEY}&format=json"
        )
        ...
        for tag in artist_info["top_tags"][:3]:
            response = r.get(
                f"http://ws.audioscrobbler.com/2.0/?method=tag.gettopartists&tag={tag}&api_key={self.API_KEY}&format=json"
            )
